...
void ProgramsWidget::run()
{
	...
	const QString programPath = projDir.filePath("bin/botball_user_program");

	// Shell-safe single-quote escaping:  abc'd  ->  'abc'"'"'d'
	QString escaped = programPath;
	escaped.replace("'", "'\"'\"'");

	const QString cmd = QString("'%1'").arg(escaped);

	ProgramWidget *programWidget = new ProgramWidget(Program::instance(), device());
	const bool success = Program::instance()->start(
			"/usr/bin/script",
			QStringList({"-q", "-c", cmd, "/dev/null"}));
	if (success)
